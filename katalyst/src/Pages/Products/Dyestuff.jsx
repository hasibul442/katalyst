...
const fetchDyestuff = async() =>{
    await axios.get(`http://localhost:8000/api/dyestuffs`).then(({data})=>{
      setDyestuff(data);
      console.log(data)
    })
}
...
<>
  ...
  <section className="pt-5 texture-1">
    <div className="container" >
      <p className="basic-chemicals-title" data-aos="fade-up">Dyestuffs</p>
        ...
        <div className="row mt-5">
          ...
          <div className="col-md-6" data-aos="fade-up">
            <h1 className="product-description-title">
              <i className="fas fa-stream"></i> Dyestuffs List
            </h1>
            ...
            <div className="product-description" >
                {
                   dyestuff.length > 0 && (
                    dyestuff.map((item)=>(
                           <div key={item.id}>
                                <span className="item-hover" ><i className="fad fa-angle-right text-success"></i> {item.dyestuff_name}</span>
                           </div>
                       ))
                   ) 
                }
